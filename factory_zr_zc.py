"""
Framework POO para construcción de DAGs dinámicos de Airflow en ZC.
...
VERSION 1: Soporte para ejecución event-driven con datasets de otros DAGs.
"""
...
@dataclass
class EventDAGMetadata:
    """Metadata del DAG - TODOS los parámetros son obligatorios"""

    dag_id: str
    codigo_malla: str
    schedule: str
    # Usuario envía lista de dicts, se convierten a EventFlujoConfig internamente
    flujos: List[Dict]
    tags: List[str]
    description: str
    owner: str
    ...
    def __post_init__(self):
        ...
        # Convertir dicts a EventFlujoConfig
        self.flujos = [EventFlujoConfig(**flujo) for flujo in self.flujos]
...
@dataclass
class EventFlujoConfig:
    """
    Configuración de un flujo de ingesta event-driven.
    dataset_origen permite vincular la dependencia con el DAG productor.
    """
...
# ============================================================
# CLASE HIJA: ROCKET DAG FACTORY EVENT-DRIVEN
# ============================================================

class EventRocketDAGFactory(BaseDAGFactory):
    """
    Clase hija especializada para DAGs con RocketOperator.
    Implementa la lógica específica para tareas de ingesta event-driven.
    """
    ...
    def _build_dataset_schedule(self) -> List[Dataset]:
        dataset_paths = {
            self.build_dataset_path(flujo.dataset_origen) for flujo in self.metadata.flujos
        }
    ...
    def _build_optional_extra_params(self, flujo: EventFlujoConfig) -> List[Dict[str, str]]:
    ...
    def _build_extra_params(self, flujo: EventFlujoConfig) -> List[Dict[str, str]]:
    ...
    def _build_rocket_operator(self, flujo: EventFlujoConfig) -> RocketOperator:
    ...
    def _build_dataset_filter(self, nombre_tabla: str, dataset_origen: str) -> ShortCircuitOperator:
    ...
    def _build_task_group(
        self,
        flujo: EventFlujoConfig,
    ) -> Tuple[ShortCircuitOperator, Optional[TimeSensor], RocketOperator, DummyOperator]:
    ...
    def _set_task_chain(
        self,
        dataset_filter: ShortCircuitOperator,
...
def create_dag(
    ...
    metadata = EventDAGMetadata(**config)
    factory = EventRocketDAGFactory(metadata, global_config, operator_config)
